...
func (c *Client) MakeRequest(
	...
	if parseErr != nil {
		slog.Error("couldn't parse url", "url", requestURL, "err", slog.String("error", parseErr.Error()))
		return "", 0
	}
	...
	if reqErr != nil {
		slog.Error("couldn't create request", "url", requestURL, "method", method, "err", slog.String("error", reqErr.Error()))
		return "", 0
	}

	c.headerMu.Lock()
	request.Header.Set("user-agent", c.userAgent)
	if c.authToken != "" && !strings.Contains(requestURL, "/oauth/internal/token") {
	...
	if len(headers) > 0 {
		for name, values := range headers {
			...
			if strings.ToLower(name) == "authorization" && c.authToken != "" && !strings.Contains(requestURL, "/oauth/internal/token") {
	...
	if doErr != nil {
		slog.Error("couldn't make request", "url", requestURL, "method", method, "err", slog.String("error", doErr.Error()))
	...
	if response == nil {
		slog.Error("received nil response from http client (and nil error)", "url", requestURL, "method", method)
		return "", 0 // Or a more specific error status code
	}

	// Handle cases where response.Body might be nil (e.g., 204 No Content)
	if response.Body == nil {
		slog.Warn("response body is nil", "url", requestURL, "method", method, "statusCode", response.StatusCode)
	...
	if readErr != nil {
		slog.Error("couldn't read response body", "url", requestURL, "method", method, "statusCode", response.StatusCode, "err", slog.String("error", readErr.Error()))
